...
impl MemoryAccessor for HookedProcess {
    ...
    fn as_direct_3d_device(&self, address: usize) -> Result<&'static IDirect3DDevice9> {
        unsafe {
    ...
    unsafe fn virtual_protect(
        ...
        VirtualProtect((self.base_addr + addr) as _, size, protect, &mut old)?;
        Ok(old)
    }
