...
### TCP Signaling (TCP でシグナリングする接続)

接続サーバーを使わず、TCP で直接接続情報を交換する方式です。  
接続待ちの間に他の機能を使用できます。  
既定では STUN サーバーを使用するため、シグナリングサーバーの提供終了後もこの方式で対戦できます。
「Offline Mode」を ON にすると STUN サーバーを使わなくなるため、
LAN 内などインターネットに接続できない環境でも便利に使えます。

接続待ち中に「Leave」を選択すると接続待ちが中断され、キャンセルボタンを押すと他の機能を使用できます。

#### TCP Signaling での対戦の仕方

1. 双方で「Ju.N.Owen」→「TCP Signaling」を選択します
2. 「Change Address」を選択し、待ち受ける側のアドレスを `192.168.0.2:19000` のように入力します
    - 待ち受ける側はポート番号のみを使用し、全てのアドレスで待ち受けます
    - インターネットに接続できない環境(LAN 内など)では「Offline Mode: OFF」を選択して
      「ON」に切り替えます。STUN サーバーへの応答を待たなくなります
3. 待ち受ける側で「Connect as a Host」を、もう片方で「Connect as a Guest」を選択します
