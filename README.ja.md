...
接続サーバーを使わず、LAN 内で TCP を使って直接接続情報を交換する方式です。  
既定では STUN サーバーも使用しない(「Offline Mode: ON」)ため、インターネットに接続できないネットワークでも動作します。

#### LAN での対戦の仕方

1. 双方で「Ju.N.Owen」→「LAN」を選択します
2. 「Change Address」を選択し、待ち受ける側のアドレスを `192.168.0.2:19000` のように入力します
    - 待ち受ける側はポート番号のみを使用し、全てのアドレスで待ち受けます
    - 双方がインターネットに接続できる場合は「Offline Mode: ON」を選択して「OFF」に切り替えます。
      STUN サーバーを使用するようになり、シグナリングサーバーの提供終了後もこの方式で対戦できます
